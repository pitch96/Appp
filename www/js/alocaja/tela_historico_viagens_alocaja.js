 ...
 open_transporte_agendado: function(transporte_id){
        ...
        let success = function (msg) {
            ...
            if (msg.status == 1) {
                ...
                util.add_path_to_breadcrumb('alocaja/tela_historico_viagens_alocaja.html');
    ...
    load_entregas_to_grid: function(){
      var count = 0;
    var cnpjFiltro = window.localStorage.getItem("filter_alocaja_cnpj_historico");
    var statusFiltro = window.localStorage.getItem("filter_alocaja_status_historico");
    console.log("FILTRADO CNPJFILTRO");
    console.log("FILTRADO STATUSFILTRO");
    console.log(cnpjFiltro);
...
*/
            let row = `<div class = "quadrante-lista"><li id="${i}" onclick="app.onClickGrid(${i});"  class="list-item with-second-label ${(i % 2 ? 'bg-gray-50' : '')}">` +
             ...
             if(((statusFiltro != '' || statusFiltro != null) && (statusFiltro == item.status)) || (statusFiltro == '' || statusFiltro == null) ) {
                count = count +1;
